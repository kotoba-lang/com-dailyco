(ns dailyco.main
  "Kotodama WASM entrypoint for the Dailyco clean-room actor (L4) — Clojure port.
  ...
  facts (`dailyco.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dailyco")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dailyco.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dailyco-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dailyco-compat/ready)
